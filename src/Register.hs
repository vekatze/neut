...
edgeInfo (meta :< (CodeCall _ label _ cont)) = do
  fun <- lookupFunEnv label >>= liftIO . readIORef
  funElems <- edgeInfo fun
  contElems <- edgeInfo cont
  return $ codeMetaLive meta : (funElems ++ contElems)
edgeInfo (meta :< (CodeJump labelName _)) = do
