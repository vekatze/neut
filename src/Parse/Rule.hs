...
-- [ WeakStmtLet (m {metaIsReducible = False}) at' indType,
--   WeakStmtLetBypass m caseIdent caseBody
...
--   --   discern
--   --     (m, WeakTermPiIntro indArgs (m, WeakTermPiElim (toVar' zt) (map toVar' atsbts)))
--   -- ats' <- mapM (discernIdentPlus . textPlusToWeakIdentPlus) ats
--   -- p "at':"
--   -- p' at'
--   p "caseBody-base:"
--   p' $ T.unpack $ toText (m, WeakTermPiIntro indArgs (m, WeakTermPiElim (toVar' zt) (map toVar' atsbts)))
--   p "caseBody:"
--   caseBody <-
--     discern
--       (m, WeakTermPiIntro indArgs (m, WeakTermPiElim (toVar' zt) (map toVar' atsbts)))
--   p $ T.unpack $ toText $ caseBody
--   -- p "atsbts':"
--   -- p' atsbts'
--   -- p "bts-new:"
--   -- p' $ drop (length xts + 2) indArgs'
--   -- bts'をcastする必要がある。
--   caseIdent <-
--     discernIdentPlus
--       (m, asIdent $ ai <> nsSep <> "case", (m, WeakTermPi indArgs univVar))
--   -- foldIdent@(_, _, foldType) <-
--   --   discernIdentPlus
--   --     (m, asIdent $ ai <> nsSep <> "fold", (m, WeakTermPi indArgs cod))
--   -- p "caseType: "
--   -- p $ T.unpack $ toText caseType
--   -- p "caseBody: "
--   -- p $ T.unpack $ toText caseBody
--   return
--     [ WeakStmtLet (m {metaIsReducible = False}) at' indType,
...
toInductiveIntro ats bts xts ai (mb, bi, m, yts, cod)
  | (_, WeakTermPiElim (_, WeakTermUpsilon a') es) <- cod,
    ...
    -- p "constructor:"
    -- p' constructorIdent
    -- bypass the cast of `yts`
    return [WeakStmtLetBypass m constructorIdent constructor]
  -- case constructor of
  --   (_, WeakTermPiIntro xtsyts (_, WeakTermPiIntro atsbts (_, WeakTermPiElim b@(_, WeakTermUpsilon bInner) _))) -> do
  --     -- p "outer (yts):"
  --     let outer = drop (length xts') xtsyts
  --     -- p' outer
  --     -- p "inner (atsbts):"
  --     let inner = (\(Just (_, _, (_, WeakTermPi dom _))) -> dom) $ find (\(_, y, _) -> y == bInner) atsbts
  --     -- p' inner
  --     -- p "cast info:"
  --     let castedArgs = foo $ zip outer $ map (\(_, _, t) -> t) inner
  --     -- p' castedArgs
  --     -- p "(constructor):"
  --     -- p' b
  ...
  --               xtsyts
  --               ( m,
  --                 WeakTermPiIntro atsbts (m, WeakTermPiElim b (map toVar' outer))
  --               )
  --           )
  --       ]
  -- return
