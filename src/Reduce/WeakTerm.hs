...
--   | Just op <- asUnaryOpMaybe constant
--   , [arg] <- es = reduceWeakTermPlusUnary orig arg m op
--   | Just op <- asBinaryOpMaybe constant
--   , [arg1, arg2] <- es = reduceWeakTermPlusBinary orig arg1 arg2 m op
--   | otherwise = orig
-- reduceWeakTermPlusUnary ::
--      WeakTermPlus -> WeakTermPlus -> Meta -> UnaryOp -> WeakTermPlus
-- reduceWeakTermPlusUnary orig arg m op = do
--   -- case getUnaryArgInfo lowType arg of
--   case arg of
--     (_, WeakTermEnumIntro (EnumValueIntS s1 x) -> do
--       case op of
--         UnaryOpTrunc lowType (LowTypeIntS s2)
--           | s1 > s2 ->
--             (m, WeakTermEnumIntro (EnumValueIntS s2 (x .&. (2 ^ s2 - 1)))) -- e.g. trunc 257 to i8 ~> 257 .&. 255 ~> 1
--         UnaryOpZext lowType (LowTypeIntS s2)
--           | s1 < s2
...
--   -> BinaryOp
--   -> WeakTermPlus
-- reduceWeakTermPlusBinary orig arg1 arg2 m op = do
