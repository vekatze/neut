...
data Pos
  ...
  | PosEpsilon Identifier
  | PosEpsilonIntro Literal
                    LowType
  | PosDownPi [(Identifier, PosPlus)]
              NegPlus
  | PosSigma [(Identifier, PosPlus)]
             PosPlus
  | PosSigmaIntro [PosPlus]

data Neg
  = NegEpsilonElim (Identifier, PosPlus)
                   PosPlus
                   [(Case, NegPlus)]
  | NegPiElimDownElim PosPlus
                      [PosPlus]
  | NegSigmaElim [(Identifier, PosPlus)]
                 PosPlus
                 NegPlus
  | NegUp PosPlus
  | NegUpIntro PosPlus
  | NegUpElim (Identifier, PosPlus)
              NegPlus
              NegPlus
...
data PosMeta
  = PosMetaTerminal (Maybe (Int, Int))
  | PosMetaNonTerminal Pos
                       (Maybe (Int, Int))

type NegMeta = String -- data NegPlus =

type PosPlus = (PosMeta, Pos)

type NegPlus = (NegMeta, Neg)
-- data Value =
--   Value PosMeta
--         Pos
--   deriving (Show)
