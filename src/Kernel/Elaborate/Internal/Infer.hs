...
extendHandle (mx, k, x, t) h =
  if isHole x then h else h {varEnv = (mx, k, x, t) : varEnv h}
...
infer h term =
  case term of
    ...
    m :< WT.BoxElim castSeq mxt e1 uncastSeq e2 -> do
      ...
      (e1', t1) <- infer h e1
      mxt'@(mx, _, _, t1') <- inferBinder1 h mxt
    ...
    m :< WT.Prim prim ->
      case prim of
        ...
        WPV.StaticString t text -> do
          t' <- inferType (h {varEnv = []}) t
          return (m :< WT.Prim (WPV.StaticString t' text), m :< WT.BoxNoema t')
        WPV.Text t text -> do
          t' <- inferType (h {varEnv = []}) t
          return (m :< WT.Prim (WPV.Text t' text), t')
        WPV.Rune _ -> do
    ...
    m :< WT.Magic (M.WeakMagic magic) -> do
      case magic of
        ...
        M.Calloc sizeType num size -> do
          ...
          liftIO $ Constraint.insert (constraintHandle h) sizeType' actualNumType
          liftIO $ Constraint.insert (constraintHandle h) sizeType' actualSizeType
          return (m :< WT.Magic (M.WeakMagic $ M.Calloc sizeType' num' size'), m :< WT.PrimType PT.Pointer)
        ...
        M.Free unitType ptr -> do
          ...
          (ptr', ptrType) <- infer h ptr
          liftIO $ Constraint.insert (constraintHandle h) (m :< WT.PrimType PT.Pointer) ptrType
          return (m :< WT.Magic (M.WeakMagic $ M.Free unitType' ptr'), unitType')
  ...
  case ty of
    ...
    m :< WT.TyApp t args -> do
      ...
      case k' of
        _ :< WT.Pi _ impParams expParams _ cod -> do
          let impParamIds = map (\(_, _, x, _) -> x) impParams
          ...
          forM_ (zip expParams' argKinds) $ \((_, _, _, tParam), tArg) ->
    ...
    m :< WT.TypeHole holeID _ -> do
      ...
      case mHoleInfo of
        ...
        Nothing -> do
          let holeArgs = map (\(mx, _, x, _) -> mx :< WT.TVar x) (varEnv h)
...
inferImpBinderWithDefaults h binderList =
  case binderList of
    ...
    ((mx, k, x, t), defaultValue) : rest -> do
      ...
      return (((mx, k, x, t'), defaultValue') : rest', h')
...
inferQuoteSeq h letSeq castDirection = do
  ...
  xts' <- inferBinder'' h xts
  (es', ts) <- mapAndUnzipM (infer h) es
  forM_ (zip xts' ts) $ \((m1, _, _, tInner), tOuter@(m2 :< _)) -> do
...
inferArgsTypes h sub m args params =
  case (args, params) of
    ([], []) ->
      return sub
    ((ty, tyKind) : rest, (_, _, x, paramKind) : xts) -> do
...
inferArgsTerms h sub m args params cod =
  case (args, params) of
    ...
    ((_, argType) : rest, (_, _, _, paramType) : xts) -> do
...
inferPiElim h m (e, t) impArgs defaultArgsSpec expArgs = do
  ...
  where
    inferCore mkKind impArgsParam expParams defaultParams cod = do
      ...
      forM_ (zip defaultParams' defaultArgsOverrides) $ \((_, _, _, tParam), mOverride) -> do
        case mOverride of
          Just (_, tArg) -> do
...
createImpArgFromParam h m (_, _, _, _paramKind) = do
...
newTypedHole h m varEnv = do
  ...
  let holeArgs = map (\(mx, _, x, _) -> mx :< WT.TVar x) varEnv
