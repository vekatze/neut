  ...
  case lowComp of
    ...
    LC.Cont op cont -> do
      ...
      case op of
        ...
        _ ->
          scenario
    LC.Switch _ (_, defaultBranch) cles (_, cont) -> do
  ...
  case lowComp of
    ...
    LC.Cont op cont -> do
      ...
      case op of
        ...
        _ ->
          LC.Cont op cont'
    LC.Switch d (defaultLabel, defaultBranch) cles (phi, cont) -> do
      let defaultBranch' = cancel' ctx defaultBranch
      let (cs, les) = unzip cles
      let (ls, es) = unzip les
      let es' = map (cancel' ctx) es
      let cont' = cancel' ctx cont
      LC.Switch d (defaultLabel, defaultBranch') (zip cs (zip ls es')) (phi, cont')
    LC.TailCall {} ->
      lowComp
    LC.Unreachable ->
      lowComp
