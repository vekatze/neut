...
returnTypeValueIntValue h m moduleID typeValue = do
  ...
  case typeValue of
    TypeValue.Algebraic dataName dataArgs consInfoList -> do
      ...
      let stringType = makeTextTypeExpr m moduleID
      let dataNameTerm = m :< TM.Prim (PV.StaticText stringType dataName)
    ...
    TypeValue.Enum consNames -> do
      ...
      let nameTerms = map (\name -> m :< TM.Prim (PV.StaticText stringType name)) consNames
...
constructConstructorTerm h m moduleID (consName, isConstLike, params) = do
  ...
  let consNameText = m :< TM.Prim (PV.StaticText stringType consName)
...
constructParamPairTerm h m moduleID stringType (paramName, paramType) = do
  let nameTerm = m :< TM.Prim (PV.StaticText stringType paramName)
...
evaluateTextCons h m stringTypeExpr rune text = do
  case (rune, text) of
    (_ :< TM.Prim (PV.Rune r), _ :< TM.Prim (PV.StaticText _ textValue)) -> do
      ...
      return $ m :< TM.Prim (PV.StaticText stringTypeExpr newText)
    _ ->
    ...
    _ :< TM.Prim (PV.StaticText stringTypeExpr textValue) -> do
      ...
      case T.uncons textValue of
        ...
        Just (c, rest) -> do
          ...
          let restText = m :< TM.Prim (PV.StaticText stringTypeExpr rest)
