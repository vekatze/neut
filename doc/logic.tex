...
\begin{framed}
  \begin{center}
    ...
    %   \RightLabel{\( (\Pi_{\ir{i}}) \)}
    %   \UnaryInfC{\( \Gamma \vdash \abs{x}{e} : \dprod{x : N}{M} \)}
    % \end{bprooftree}
    \begin{bprooftree}
      \AxiomC{\( \Gamma, x : N \vdash e : M \)}
      \RightLabel{\( (\Pi_{\ir{i}}) \)}
      \UnaryInfC{\( \Gamma \vdash \vret{(\thunkS{(\abs{x}{e})})} : \upT{\downT{\dprod{x : N}{M}}} \)}
    \end{bprooftree}
    \\
    \vspace{1.5em}
    \begin{bprooftree}
      \AxiomC{\( \Gamma \vdash e_{1} : \upT{\downT{\dprod{x : N}{M}}} \)}
      \AxiomC{\( \Gamma \vdash e_{2} : \downT{N} \)}
      \RightLabel{\( (\Pi_{\ir{e}}) \)}
      \BinaryInfC{\( \Gamma \vdash \bind{e_{1}}{f}{\appE{(\forceS{f})}{e_{2}}} : M \subst{x}{e_{1}} \)}
    \end{bprooftree}
    \\
    \vspace{1.5em}
    \begin{bprooftree}
