...
By running =$ neut build --no-alloc-cancellation --emit llvm filename.neut=, we obtain the following LLVM IR (the =--no-alloc-cancellation= is to disable certain optimization that we will see [[#memory-reuse-via-linearity][later]]):
#+BEGIN_SRC llvm
