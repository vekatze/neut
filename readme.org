...
That is a valid question. The key to give an answer to this question is generalizing the concept of "all the free variables". Consider the following term:
#+BEGIN_SRC haskell
...
Note that every result of this procedure is necessarily "closed". That is, if =[x1 : A1, ..., xn : An]= is a result of this tracing process, then the set of free variables in =A{i}= is exhausted by ={x1, ..., x{i-1}}=. In this way, we extract a closed chain from a lambda-abstraction.
...
cartesian-closure i closure = do
  -- extract the element of the closure, freeing the outer tuple
  let (typeOfChain, chain, label) := closure in
  if i == 0
  then do
    ...
    let (chain_A, chain_B) := typeOfChain(1, chain) in
    -- construct the 2 closures, and the wrapper tuple (thus do malloc for the 3 times)
    return ((typeOfChain, chain_A, label), (typeOfChain, chain_B, label))
#+END_SRC
Thus, we can translate every Π-type into the function =cartesian-closure=. Every Closure is copied/discarded in the same way, regardless of its actual implementation. So, information like =i64= or =bool= in =i64 -> bool= is indeed useless here; But it isn't a problem since every closure knows how to copy/discard itself.
