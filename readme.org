...
- [[#examples][Examples]]
  - [[#basic-behavior-of-proof-theoretic-memory-management][Basic Behavior of Proof-Theoretic Memory Management]]
...
** Basic Behavior of Proof-Theoretic Memory Management
...
#+BEGIN_SRC txt
let str : String := "a";
<THE CODE THAT USES `str` for the 3 times>
#+END_SRC
...
Incidentally,in actual implementation, the result of =TRANSLATE(String)= is not a pair, but a function as in the pseudo-code below:
#+BEGIN_SRC txt
cartesian_string :: (i : Int, _ : String) -> if i == 0 then Top else String * String
cartesian_string (i, e) :=
  if i == 0
  then discard-string(e)
  else copy-string(e)
#+END_SRC
By translating type in this manner, we can omit creating tuples every time when we translate a type. Thus, in the actual implementation (you can find it in =src/Clarify.hs=), a type is translated into a closed function, which is represented in 1 word (a pointer to the function).
...
That is a valid question - And here comes dependent-type. Firstly, note that a polymorphic function in dependent-type theory is nothing but an ordinary function with an argument of type =type=. For example, the following is a polymorphic function that creates a pair of any type:
#+BEGIN_SRC neut
(define to-tuple ((a tau) (x a))
  (tuple x x))
#+END_SRC
where the =tau= is the type of types. This function =to-tuple= is, for example, used as follows:
#+BEGIN_SRC neut
...
;; compare:
(define to-tuple-i64 ((x i64)) (tuple x x))
(to-tuple-i64 1) ; ~> (tuple 1 1)
#+END_SRC
...
This is the basics of the behavior of proof-theoretic memory management. As you can easily imagine, this naive copying/discarding can result in an inefficient object code. We often use a variable more than once, as in the example of =str=. We can't say that code is efficient enough; We can't ignore those redundant copy operations.
