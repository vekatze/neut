...
** erase
=erase= removes specified bound variables from its scope.
*** Example
#+BEGIN_SRC neut
((λ (x y) x) bool.true bool.false)             ; ~> bool.true

((λ (x y) (erase (x y) x)) bool.true)          ; ~> undefined variable: x

; `with` notation has dedicated notation for `erase`
(with identity.bind
  (let x bool.true)
  (let y x)
  (erase x)
...
(with identity.bind
  ...
  (erase var)
  {REST_COMPUTATION_WITHOUT_VAR}
  ...)
#+END_SRC
This might be useful for debug purposes.

 =erase= can also be used to ensure that a variable introduced in [[#notation][notation]] doesn't cause name collision to a free variable. For example, consider the following notation:
#+BEGIN_SRC neut
...
#+END_SRC
That is, using =erase=, we can ensure that a variable introduced by =notation= does not collide to a free variable contained in an argument of the notation.

Note that =erase= is not a complete solution to the problem of [[https://en.wikipedia.org/wiki/Hygienic_macro][hygienic macros]]: consider the following code:
#+BEGIN_SRC neut
(notation (unless condition on-false on-true)
  (if (bool.not condition) on-true on-false)
#+END_SRC
The notation above presupposes that the free variable =bool.not= is bound in a sane way; =bool.not= can't be something like =(λ (x) x)=, or =(string.print "hello, world!")=. =erase= has no use in this situation, though we might be able to introduce something like =(coerase (LEAF ... LEAF) TREE)= which ensures that every given variable is defined at top level.
...
* Auxiliary Syntax
** asterisk
=*= is a placeholder that should be inferred.
*** Example
#+BEGIN_SRC neut
(define id ((a tau) (x a)) x)
