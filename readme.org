...
- [[#appendix][Appendix]]
  ...
  - [[#types-as-exponentials-more-specifically][Types as Exponentials, More Specifically]]
...
** Syscalls
Neut supports syscalls, although its current support is quite limited. As you can see in =core/VERSION/constant/{linux, darwin}.neut=, only basic syscalls like =read, write, exit, open, close= are supported. This is not something that caused by some theoretical difficulties; Rather, it is simply because of my limited resources.

Syscall can be added via the following procedure:
1. Declare corresponding constant (like =os.write=) using the [[#constant][constant]] statement
2. Register argument information and syscall number in =src/Data/Syscall.hs=
3. Rebuild the compiler and run some test codes to check if it really works

Note that a syscall shouldn't produce/consume a resource. For example, the syscall =write= is (currently) declared to have the following type in Neut:
#+BEGIN_SRC neut
(constant write
  (pi
    ((A i64)
...
** Types as Exponentials, More Specifically
