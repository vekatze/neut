...
Note that the result of this process is necesarrily "closed". That is, if =[x1 : A1, ..., xn : An]= is a result of this tracing process, then the set of free variables in =A{i}= is exhausted by ={x1, ..., x{i-1}}=. In this way, we extract a closed chain from a lambda-abstraction.
...
#+BEGIN_SRC text
(Sigma [a : type, x : a], (a, x), LABEL_TO_A_CLOSED_FUNCTION)
#+END_SRC
That is, a closure is translated into *(0) the type of the closed chain*, (1) the closed chain, and (2) a pointer to an appropriately-arranged closed function. Now, remember that every type gives the way how to copy/discard the terms of the type. Also remember that every type is translated into a function pointer, which can be copied/discarded without any malloc/free operations. Thus, we can generate a function that copies/discards a closure conceptually as follows:
#+BEGIN_SRC text
...
The remaining piece is how a type of a closed chain like =Sigma [a : type, x : a]= is translated.

# The key is the "all the" part of (1).

# Proof-theoretically, this can be justified via the following type-isomorphism: Pi (x : A). B = Sigma (C : tau). C * Box (Pi (x : A, env :  C). B), where the =Box= is a necessity modality which can only be introduced when the antecedent is empty.
