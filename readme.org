...
cartesian-closure i closure = do
  ...
  then do
    ...
    let (chain_A, chain_B) := typeOfChain(1, chain);
...
Regarding macOS: You may note that the "syscalls" are lowered to some external interface functions on macOS. This is because macOS (>= 10.12) doesn't support using syscalls directly; Indeed, for example, if we were to use the syscall =fork= directly (0x2000002), succeeding =malloc= causes a fatal error when I tested, saying =mach_vm_map(size=1048576) failed (error code=268435459)=.
