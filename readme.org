...
(reduce
  ...
  top.unit)
#+end_src
The code prints =hello= twice since the =define= does not reduce the body of the variable.

# If a user input =e= is not parsed as a statement when it is supposed to be, the compiler interprets it as =(let _ e)=.

*** define-prefix, remove-prefix
The behaviors of =define-prefix= and =remove-prefix= are the same as the ones of the meta language.
