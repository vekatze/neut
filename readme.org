...
- [[#appendix][Appendix]]
  ...
  - [[#proof-theoretic-memory-management-more-specifically][Proof-Theoretic Memory Management, More Specifically]]
...
#+END_SRC
Note that the second line uses the pair =(discard-string, copy-string)=, which is the result of translation of the type =string=. Every type =X= is translated in this manner; the pair of =discard-X= and =copy-X=. These exponential functions - something that allow us to create n copies of =x= from a single =x= - are used in its continuation so that every variable of this type (=str= in this example) is used linearly.
...
That is a valid question. The key to give an answer to this question is generalizing the concept of "all the free variables". Consider the following pseudo-code:
#+BEGIN_SRC haskell
let make-pair-of-any-and-i64 := λ (a : type). λ (x : a). λ (y : i64). (x, y)
#+END_SRC
In ordinary closure conversion, the free variables of =λ (y : i64). (x, y)= is calculated to be =[x]= without making a fuss. Here, however, we generalize the concept to "trace" all the free variables included in the type of every free variable. In this case, for example, note that the type of =x= is =a=, which is again a free variable if it occurs in =λ (y : i64). (...)=, and thus this =a= is also considered to be a free variable. Since the type of =a= is =type=, which has no free variables, our tracing stops here, resulting a chain of free variables =[a : type, x : a]=.

Note that the result of this process is necessarily "closed". That is, if =[x1 : A1, ..., xn : An]= is a result of this tracing process, then the set of free variables in =A{i}= is exhausted by ={x1, ..., x{i-1}}=. In this way, we extract a closed chain from a lambda-abstraction.
...
#+END_SRC
That is, a closure is translated into *(0) the type of the closed chain*, (1) the closed chain, and (2) a pointer to an appropriately-arranged closed function. Now, remember that every type is translated into a term that copies/discards the terms of the type. Also remember that every type is translated into a function pointer, which can be copied/discarded without any malloc/free operations. Thus, we can generate a function that copies/discards a closure conceptually as follows:
#+BEGIN_SRC haskell
cartesian-closure i closure = do
  let (TYPE_OF_CHAIN, CHAIN, LABEL) := closure in -- extract the element of the closure, freeing the outer tuple
  if i == 0
  then do
    let _ := TYPE_OF_CHAIN(0, CHAIN); -- discard the chain using the type information
    return ()                         -- note that both TYPE_OF_CHAIN and LABEL are immediate
  else do
    let (CHAIN_A, CHAIN_B) := TYPE_OF_CHAIN(1, CHAIN) in
    return ((TYPE_OF_CHAIN, CHAIN_A, LABEL),
            (TYPE_OF_CHAIN, CHAIN_B, LABEL)) -- construct the 2 closures, and the wrapper tuple (thus do malloc for the 3 times)
#+END_SRC
Thus, we can translate every Π-type into the function =cartesian-closure=. Every Closure is copied/discarded in the same way, regardless of its actual implementation. So, type information like =i64 -> bool= is indeed useless here; But it isn't a problem since every closure knows how to copy/discard itself.

The remaining piece is how a type of a closed chain like =∑ [a : type, x : a]= is translated. This is where eta-expansion plays its role. Suppose we have a term =e= of type =∑ [a : type, x : a]=. Since we already know that the type of =e=, we can eta-expand this term as follows:
#+BEGIN_SRC haskell
let (a, x) := e in (a, x)
#+END_SRC
...
#+END_SRC
which is valid since a type is translated into a function pointer. This creates a term that uses both =a= and =x= linearly.
...
In conclusion, =∑ [a : type, x : a]= is translated into the following term:
#+BEGIN_SRC haskell
cartesian-sigma sig :=
  let (a, x) := sig in
  let (a1, tmp) := type(1, a) in
  ...
  ((a2, x1), (a3, x2))
#+END_SRC
By using this term, we can now copy the tuple =(a, x)= in the closure =(∑ [a : type, x : a], (a, x), LABEL)=. The discarding function is defined similarly. In this way we can copy/discard a closure.

(Here one might be lead to say that this translation is a generalized (n-ary) eta-expansion)
...
The point of the workaround is straightforward: If those copying/discarding result from using variables in non-linear way, we simply have to use those variables in linear way. Let's go back to the first example code:
#+BEGIN_SRC neut
(ensure core/0.1.0.0
  "https://github.com/u2zv1wx/neut-core/archive/0.1.0.0.tar.gz")
...
# ** erase
# The =erase= statement makes given variable invisible in its continuation.
# *** Example
# #+BEGIN_SRC neut
# (define helper-function (x) (add-i64 x 1))

# (define public-function (x) (mul-i64 (helper-function x) 2))

# (erase helper-function)

# (public-function 10)   ; ~> 22

# ; (helper-function 10) ; ~> undefined variable: helper-function
# #+END_SRC
# *** Syntax
# #+BEGIN_SRC neut
# (erase LEAF)
...
# The =erase= statement makes given variable invisible in its continuation.
# *** Notes
# =erase= can be used to realize a private function. If you
...
Regarding macOS: You may note that the "syscalls" are lowered to some external interface functions on macOS. This is because macOS (>= 10.12) doesn't support using syscalls directly; Indeed, for example, if we were to use the syscall =fork= directly (0x2000002), succeeding =malloc= causes a fatal error, saying =mach_vm_map(size=1048576) failed (error code=268435459)=.

** Array Indexing
Neut offers a family of constants that allows accessing an element of an array. The array is of the form ={LOWTYPE}.array-access=, where the ={LOWTYPE}= is either =i{n}= or =f{n}=. For example, =i8.array-access= can be used for indexing an array of type =(array N i8)=.
...
** Unsafe Cast
The constant =unsafe.cast : Pi (A : tau, B : tau, x : A). B= is available (assuming that the constant is declared using the statement [[#constant][constant]]). The semantics of this constant is no-op cast:
#+BEGIN_SRC neut
(unsafe.cast t1 t2 e)
~> e
#+END_SRC
...
** Proof-Theoretic Memory Management, More Specifically
