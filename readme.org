...
*** Unsafe Cast
The constant =unsafe.cast : Pi (A : tau, B : tau, x : A). B= is available. The semantics of this constant is specified as follows:
#+BEGIN_SRC neut
(unsafe.cast t1 t2 e)
~> e
#+END_SRC
That is, =unsafe.cast= is a no-op cast operation.
...
(let bar z) ; (*2)
#+END_SRC
The code above uses the variable =z= for the two times. Since =z= is casted to a Pi-type, The resulting code tries to copy the value =1= as if it were a closure. Since the internal representation of closure at least uses 3 words (the type of all the free variables, all the free variables, and the label of a closed function), this causes an access to an invalid memory region, that is, a segmentation fault.
