...
The basic characteristics of Neut can be summerized as follows:

- Compiled language
- Output: LLVM IR / assembly / object binary
- Type system: CoC + recursion + int + float + enum + array + struct
- Evaluation strategy: call-by-value
- Memory management: statically determined at compile time
  - Can be optimized via borrowing and linearity
- Has inductive/coinductive type
