...
#+END_SRC
Again, the non-schematic output can be found in the [[#llvm-ir-for-the-second-example][appendix]]. The output in this time is actually short enough to include it here (36 lines), though I chose not to. The point here is that the string "a" is reused without copying, as expected.

 The resulting assembly code is reasonably small too (works on macOS; when you compile the same code in Linux, the =write= operation is lowered into a syscall):
#+BEGIN_SRC asm
