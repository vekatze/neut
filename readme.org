...
But if the size of memory being allocated/deallocated is known at compile-time, isn't it possible to compare the size of them in compile-time and emit a code that reuses the allocated memory?
...
#+END_SRC
In other words, the compiler can cancel the memory deallocation at =(*1)= and the allocation at =(*2)=, reusing the allocated memory in its continuation. This is automatic malloc/free cancellation. By this "fallback" optimization, the compiler can emit performant code even if a user wrote code in "inefficient" way.

Note that the "creating string" parts are not optimized away in the resulting LLVM IR, in contrast to the one of borrowing:
#+BEGIN_SRC llvm
define i64 @main() {
  ;; <memory allocation for the string>
  ;; <create the string "a" (i.e. write the character 'a' to the allocated memory)>
...
#+END_SRC
Although the compiler can cancel memory allocations/deallocations, it cannot cancel its accompanying initialization process. It might also be possible to optimize away these initializations, but at least currently it is not possible. If you do need performance, you need to write code in the linear/borrowing style.

# , or Mental Model of Memory Management in Neut
** Summary
...
- Borrowing can be used as a convenient syntactic notation when accomplishing linearity.
- Resulting code is free from redundant malloc/free thanks to automatic malloc/free cancellation.
