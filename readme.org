...
#+END_SRC
*** Semantics
When evaluated, the =(attribute x attr)= statement modifies the state of compiler so that the variable =x= has specified attribute =attr=. Currently, the only attribute that can be specified is =(implicit NUM ... NUM)=.

If the implicit information of a variable =x= is set to be =(implicit NUM ... NUM)=, the compiler inserts holes at the specified indices of the arguments of function-application =(x e1 ... en)=.

If the variable is prefixed by =@= when its used, the implicit information is ignored.

** constant
The =constant= statement declares external constant.
...
#+END_SRC
*** Semantics
When evaluated, the =(constant x t)= statement declares constant =x= of type =t= for later use. Declared constants can then be available for use as in the same way of ordinary variables. Every constant must have distinct names. This statement is currently intended for internal use, such as for implementation of syscalls. External constants (FFI) might be supported in future release via this statement.
