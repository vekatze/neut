...
As a side note, internally, a =coinductive= statement is implemented via a translation into an =inductive= statement. For example, the example of =stream= above is translated into the following +BEGIN_SRC:
#statement scheme
