...
A program of Neut is a list of statements. Each statement is one of the following statements described in this section. Note that this section is intended to be a reference, and not a tutorial.

Every example in this section is a complete program in itself. You can [[#build][build]] and run the code to check the behavior.
...
#+END_SRC
*** Semantics
When evaluated, the =(constant x t)= statement declares a [[#constant][constant]] =x= of type =t= for later use. Declared constants can then be available for use as in the same way of ordinary [[#upsilon][variables]]. Every constant must have distinct names. This statement is currently intended for internal use, such as for implementation of syscalls. External constants (FFI) might also be supported in future release via this statement.
